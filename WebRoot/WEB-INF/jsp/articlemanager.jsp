...
<script type="text/javascript">
	...
	app.controller('articleController', function($scope, $http) {
		...
		$scope.findByTj = function(page,size){
			$http.post('/weshare/article/articlemanager?page='+page+'&size='+size,$scope.search).success(
				function(response){
					//显示当前页数据 
					$scope.articleList=response.rows;	
					//更新总记录数 
		...
		$scope.delarticle = function(id){
			$http.get('/weshare/article/delarticlemanager?aId='+id).success(
				function(response){
		...
		<div class="table-box">
			...
			<div class="box-tools pull-right">
				<div class="has-feedback">
					动态标题：<input ng-model="search.articletitle" type="text" />
			...
			<table id="dataList"
				...
				<thead>
					<tr>
						...
						<th class="sorting_asc">动态id</th>
						<th class="sorting">动态标题</th>
						<th class="sorting">发表动态时间</th>
					...
					<tr  ng-repeat="article in articleList">
						...
						<td class="text-center">
							<JButton type="JButton" class="btn bg-olive btn-xs"
								...
								ng-click='delarticle(article.articleid)'>删除</JButton>
						</td>
					</tr>
				</tbody>
			</table>
