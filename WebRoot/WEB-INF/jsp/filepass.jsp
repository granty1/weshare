...
<script type="text/javascript">
	...
	app.controller('fileController', function($scope, $http) {
		...
		$scope.findByTj = function(page,size){
			$http.post('/weshare/file/filepasslist?page='+page+'&size='+size,$scope.search).success(
				function(response){
					//显示当前页数据 
		...
		$scope.passfile = function(id){
			$http.get('/weshare/file/passfile?fId='+id).success(
				function(response){
					...
					<tr  ng-repeat="file in fileList">
						...
						<td class="text-center">
							<JButton type="JButton" class="btn bg-olive btn-xs"
								...
								ng-click='passfile(file.fileid)'>审核通过</JButton>
						</td>
					</tr>
				</tbody>
			</table>
